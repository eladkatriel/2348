...
MAP_PIXEL_WIDTH = 1280
MAP_PIXEL_HEIGHT = 867
TARGET_HORIZONTAL_METERS = 50.0

MAPBOX_GEOCODE_URL = "https://api.mapbox.com/search/geocode/v6/forward"
MAPBOX_STATIC_BASE = "https://api.mapbox.com/styles/v1/mapbox/satellite-v9/static"
...
def calc_zoom_for_target_width(lat: float, width_px: int, target_width_m: float) -> float:
    meters_per_pixel = target_width_m / width_px
...
def build_mapbox_static_url(lon: float, lat: float, zoom: float) -> str:
    ...
    return (
        ...
        f"{MAP_PIXEL_WIDTH}x{MAP_PIXEL_HEIGHT}@2x"
        f"?access_token={MAPBOX_ACCESS_TOKEN}&logo=false&attribution=false"
    )


def create_mapbox_satellite_image(address_text: str) -> bytes:
    if address_text in MAP_CACHE:
        print("MAP CACHE HIT")
        return MAP_CACHE[address_text]

    lon, lat = geocode_address_mapbox(address_text)
    zoom = calc_zoom_for_target_width(lat, MAP_PIXEL_WIDTH, TARGET_HORIZONTAL_METERS)
...
def create_report(data: dict):
    ...
    if address_text:
        map_bytes = create_mapbox_satellite_image(address_text)
